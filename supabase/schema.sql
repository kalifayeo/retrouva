...
insert into site_settings (cle, valeur) values
  ...
  ('hero_video_url', ''),
  ('hero_backdrop_url', ''),
  -- Mode maintenance (voir migration_24_maintenance.sql) : basculé depuis
  -- /admin/contenu, lu par middleware/00.maintenance.global.js.
...
-- Fil d'activité public (page d'accueil) : mélange pertes et trouvailles,
-- avec uniquement les champs déjà publics par ailleurs pour les
-- trouvailles (type d'objet, ville, commune, date) — jamais de
-- description, de user_id ni de critère de vérification. Inclut aussi les
-- déclarations déjà restituées (statut renvoyé pour un libellé adapté côté
-- accueil : "remis à son propriétaire" plutôt que "retrouvé").
...
-- Pas de policy d'insertion publique : la création d'un don passe par
-- server/api/dons/creer.post.js (clé service_role côté serveur), qui
-- applique une limite de fréquence par IP avant d'insérer. Voir
-- migration_27_securite_paiements.sql pour le détail du raisonnement.
...
-- Sécurité paiement (migration 27) : traçabilité fournisseur + anti-fraude
...
alter table donations add column if not exists provider_transaction_id text;
alter table donations add column if not exists verified_at timestamptz;
alter table donations add column if not exists ip_creation text;
alter table donations add column if not exists provider_metadata jsonb;
