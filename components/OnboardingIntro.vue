...
const { fonctionnelRefuse, charger: chargerConsentement } = useCookieConsent()
const slides = ref([])
const step = ref(0)
...
onMounted(async () => {
  ...
  // Respecte un refus explicite des cookies de confort (voir /cookies).
  if (fonctionnelRefuse.value) { slidesTerminees.value = true; return }
