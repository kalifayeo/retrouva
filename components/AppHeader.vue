...
const { notificationsNonLues, messagesNonLus, charger: chargerNotifications } = useUnreadCounts()
...
const menuCompte = computed(() => {
  const items = [
    ...
    { label: 'Mes dons', to: '/mes-dons', icon: 'gift' }
  ]
...
<template>
  <header class="sticky top-0 z-50 bg-ivoire-100/90 dark:bg-forest-900/95 backdrop-blur border-b border-forest-50 dark:border-forest-800 pt-safe-top">
    <div class="section container-app flex items-center justify-between h-16 gap-3">
      ...
      <form class="hidden lg:flex items-center flex-1 max-w-xs mx-2" @submit.prevent="lancerRecherche">
        <div class="flex items-center gap-2 w-full bg-white border border-forest-100 px-3.5 py-2 focus-within:border-savane-400 transition-colors">
          ...
          <input
            ...
            class="w-full text-sm outline-none bg-transparent text-ink dark:text-ivoire-100 placeholder:text-forest-300 dark:placeholder:text-forest-500"
          />
          <button
            type="submit"
            class="shrink-0 flex h-7 w-7 items-center justify-center rounded-full bg-forest-800 text-white hover:bg-forest-700 transition-colors"
            aria-label="Lancer la recherche"
          >
            <IconTab name="search" class="h-3.5 w-3.5" />
          </button>
        </div>
      </form>
      ...
      <div class="flex items-center gap-1 lg:hidden shrink-0">
        ...
        <button class="tap-target p-2 text-forest-800 dark:text-ivoire-100" @click="open = !open" aria-label="Menu">
          <span class="relative flex h-6 w-6 items-center justify-center overflow-hidden">
            <transition
              mode="out-in"
            ...
            >
              <IconTab :key="open ? 'close' : 'menu'" :name="open ? 'close' : 'menu'" class="h-6 w-6" />
            </transition>
          </span>
        </button>
      </div>
    ...
    >
      <div
        v-if="open"
        class="lg:hidden fixed inset-0 top-16 z-40 bg-forest-900/50 backdrop-blur-[2px]"
        @click="open = false"
      ></div>
    </transition>

    <transition
      enter-active-class="transition duration-300 ease-[cubic-bezier(.16,.84,.44,1)]"
    ...
    >
      <div v-if="open" class="mobile-menu-panel lg:hidden relative z-40 origin-top border-t border-forest-50 dark:border-forest-800 bg-white dark:bg-forest-900 max-h-[calc(100vh-4rem)] overflow-y-auto overscroll-contain pb-24">
        <!-- Bloc recherche -->
        <div class="px-5 pt-4 pb-4 border-b border-forest-50 dark:border-forest-800">
          <form class="flex items-center gap-2 border border-forest-100 dark:border-forest-700 px-3.5 py-2.5" @submit.prevent="lancerRecherche">
            <IconTab name="search" class="h-4 w-4 text-forest-400 shrink-0" />
            <input v-model="recherche" type="text" placeholder="Rechercher un objet…" class="w-full text-sm outline-none bg-transparent text-ink dark:text-ivoire-100 placeholder:text-forest-300 dark:placeholder:text-forest-500" />
            <button
              type="submit"
              class="shrink-0 flex h-8 w-8 items-center justify-center rounded-full bg-forest-800 text-white"
              aria-label="Lancer la recherche"
            >
