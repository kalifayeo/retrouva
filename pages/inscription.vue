...
const inscrire = async () => {
  ...
  if (!accepteConditions.value) {
    error.value = "Vous devez accepter les CGU et la politique de confidentialité pour créer un compte."
    return
  }
    ...
    <div class="auth-form-panel">
      <div class="w-full max-w-xs mx-auto">
        ...
        <form class="space-y-3.5" @submit.prevent="inscrire">
          ...
          <label class="flex items-start gap-2.5 text-xs text-forest-600 leading-relaxed">
            <input v-model="accepteConditions" type="checkbox" required class="mt-0.5 h-4 w-4 shrink-0 rounded border-forest-200 accent-forest-800" />
            <span>
              J'accepte les
              <NuxtLink to="/conditions-utilisation" target="_blank" class="font-semibold text-forest-800 dark:text-ivoire-100 underline">conditions générales d'utilisation</NuxtLink>
              et la
              <NuxtLink to="/politique-de-confidentialite" target="_blank" class="font-semibold text-forest-800 dark:text-ivoire-100 underline">politique de confidentialité</NuxtLink>.
            </span>
          </label>

          <p v-if="error" class="text-sm text-red-600">{{ error }}</p>

          <button type="submit" class="btn-primary w-full" :disabled="loading || !accepteConditions" :class="{ 'opacity-60': loading || !accepteConditions }">
