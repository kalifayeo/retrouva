...
// ---------------------------------------------------------------------
// INVITATION D'UN NOUVEL ADMINISTRATEUR — appelle la route serveur
// /api/admin/inviter (clé service_role, jamais côté client). Réservé aux
// super_administrateur : le bouton n'est même affiché qu'à eux (voir
// peutGererRoles), et le rôle de l'appelant est re-vérifié côté serveur.
// ---------------------------------------------------------------------
const config = useRuntimeConfig()
const invitationOuverte = ref(false)
const emailInvitation = ref('')
const roleInvitation = ref('moderateur')
const envoiInvitation = ref(false)
const messageInvitation = ref(null)

const envoyerInvitation = async () => {
  messageInvitation.value = null
  ...
  try {
    ...
    await $fetch(`${config.public.apiBaseUrl}/api/admin/inviter`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${jeton}` },
      body: { email: emailInvitation.value, role: roleInvitation.value }
    })

    messageInvitation.value = { type: 'succes', texte: 'Invitation envoyée ✓' }
    emailInvitation.value = ''
    await charger()
  } catch (e) {
    messageInvitation.value = { type: 'erreur', texte: e.data?.statusMessage || e.message || "Échec de l'invitation." }
  } finally {
  ...
  <div class="section py-8 md:py-12">
    <div class="container-app">
      <div class="flex items-center justify-between mb-1">
        ...
        <button v-if="peutGererRoles" class="btn-accent !py-2 !px-4 text-sm" @click="invitationOuverte = true">
          + Inviter un administrateur
        </button>
      </div>
      <p class="text-forest-700/70 mb-6">{{ utilisateurs.length }} comptes enregistrés.</p>

      <!-- Invitation d'un nouvel administrateur/modérateur -->
      <div v-if="invitationOuverte" class="card p-5 mb-6">
        <h2 class="font-display font-semibold text-sm mb-3">Inviter un nouvel administrateur</h2>
        <form class="flex flex-col sm:flex-row gap-3" @submit.prevent="envoyerInvitation">
          <input v-model="emailInvitation" type="email" class="input-field flex-1" placeholder="email@exemple.com" required />
          <select v-model="roleInvitation" class="input-field sm:!w-48">
            <option value="moderateur">modérateur</option>
            <option value="administrateur">administrateur</option>
            <option value="super_administrateur">super_administrateur</option>
          </select>
          <button type="submit" class="btn-accent !py-2.5 text-sm whitespace-nowrap" :disabled="envoiInvitation">
            {{ envoiInvitation ? 'Envoi…' : "Envoyer l'invitation" }}
          </button>
        </form>
        <p v-if="messageInvitation" class="text-xs mt-2" :class="messageInvitation.type === 'succes' ? 'text-forest-600' : 'text-red-600'">
          {{ messageInvitation.texte }}
        </p>
        <button type="button" class="text-xs text-forest-500 mt-3" @click="invitationOuverte = false">Fermer</button>
      </div>
