...
const { user, profile, signOut, signOutPartout, mettreAJourProfil, televerserAvatar, demanderChangementEmail, demanderVerificationTelephone, supprimerCompte } = useAuth()
...
const logoutPartout = async () => {
  if (!confirm('Se déconnecter de tous les appareils où vous êtes connecté ?')) return
  await signOutPartout()
  navigateTo('/')
}
...
const confirmerSuppression = async () => {
  ...
  try {
    await supprimerCompte(motDePasseSuppression.value)
    navigateTo('/')
  } catch (e) {
    erreurSuppression.value = e.message || 'Impossible de supprimer le compte.'
  } finally {
...
// puis redirige vers /verifier-nouvel-email pour la confirmer.
// ---------------------------------------------------------------------
const modificationEmail = ref(false)
const nouvelEmail = ref('')
const envoiEmail = ref(false)
const erreurEmail = ref('')

const envoyerChangementEmail = async () => {
  erreurEmail.value = ''
  envoiEmail.value = true
  try {
    await demanderChangementEmail(nouvelEmail.value)
    navigateTo({ path: '/verifier-nouvel-email', query: { email: nouvelEmail.value } })
  } catch (e) {
    erreurEmail.value = e.message || "Impossible d'envoyer le code."
  } finally {
    envoiEmail.value = false
  }
}

// ---------------------------------------------------------------------
// VÉRIFICATION DU TÉLÉPHONE PAR SMS — envoie un code, puis redirige vers
// /verifier-telephone pour le confirmer.
// ---------------------------------------------------------------------
const modificationTelephone = ref(false)
const telephoneAVerifier = ref('')
const envoiTelephone = ref(false)
const erreurTelephone = ref('')

const ouvrirVerificationTelephone = () => {
  telephoneAVerifier.value = profile.value?.telephone || ''
  modificationTelephone.value = true
}

const envoyerVerificationTelephone = async () => {
  erreurTelephone.value = ''
  envoiTelephone.value = true
  try {
    await demanderVerificationTelephone(telephoneAVerifier.value)
    navigateTo({ path: '/verifier-telephone', query: { numero: telephoneAVerifier.value } })
  } catch (e) {
    erreurTelephone.value = e.message || "Impossible d'envoyer le code."
  } finally {
    envoiTelephone.value = false
  }
}
...
<template>
  <div class="section py-8 md:py-12">
    ...
    <div class="container-app max-w-xl">
      ...
      <div class="card p-5 mb-4">
        <div class="flex items-center justify-between mb-1">
          <h2 class="font-display font-semibold text-sm">Adresse e-mail</h2>
          <button v-if="!modificationEmail" class="text-xs font-semibold text-savane-600 hover:underline" @click="modificationEmail = true">
            Changer
          </button>
        </div>

        <template v-if="!modificationEmail">
          <p class="text-sm text-forest-700/70 truncate">{{ user?.email }}</p>
        </template>

        <form v-else class="space-y-3 mt-3" @submit.prevent="envoyerChangementEmail">
          <div>
            <label class="label-field">Nouvelle adresse e-mail</label>
            <input v-model="nouvelEmail" type="email" class="input-field" placeholder="nouvelle@adresse.com" required />
          </div>
          <p v-if="erreurEmail" class="text-xs text-red-600">{{ erreurEmail }}</p>
          <div class="flex gap-2 pt-1">
            <button type="submit" class="btn-accent !py-2.5 text-sm flex-1" :disabled="envoiEmail">
              {{ envoiEmail ? 'Envoi…' : 'Envoyer un code de vérification' }}
            </button>
            <button type="button" class="btn-outline !py-2.5 text-sm" @click="modificationEmail = false">Annuler</button>
          </div>
        </form>
      </div>

      <!-- Vérifier le numéro de téléphone par SMS -->
      <div class="card p-5 mb-6">
        <div class="flex items-center justify-between mb-1">
          <h2 class="font-display font-semibold text-sm flex items-center gap-2">
            Téléphone
            <span v-if="profile?.telephone_verifie" class="badge-green !py-0.5 !px-2 text-[10px]">✓ Vérifié</span>
          </h2>
          <button v-if="!modificationTelephone && !profile?.telephone_verifie" class="text-xs font-semibold text-savane-600 hover:underline" @click="ouvrirVerificationTelephone">
            Vérifier
          </button>
        </div>

        <template v-if="!modificationTelephone">
          <p class="text-sm text-forest-700/70">{{ profile?.telephone || 'Aucun numéro renseigné.' }}</p>
        </template>

        <form v-else class="space-y-3 mt-3" @submit.prevent="envoyerVerificationTelephone">
          <div>
            <label class="label-field">Numéro de téléphone</label>
            <input v-model="telephoneAVerifier" type="tel" class="input-field" placeholder="07 00 00 00 00" required />
          </div>
          <p v-if="erreurTelephone" class="text-xs text-red-600">{{ erreurTelephone }}</p>
          <div class="flex gap-2 pt-1">
            <button type="submit" class="btn-accent !py-2.5 text-sm flex-1" :disabled="envoiTelephone">
              {{ envoiTelephone ? 'Envoi…' : 'Envoyer un code par SMS' }}
            </button>
            <button type="button" class="btn-outline !py-2.5 text-sm" @click="modificationTelephone = false">Annuler</button>
          </div>
        </form>
      </div>
      ...
      <button class="text-xs text-forest-500 w-full mt-3 text-center" @click="logoutPartout">
        Se déconnecter de tous les appareils
      </button>

      <!-- Suppression de compte -->
      <div class="mt-8 pt-6 border-t border-forest-50">
        <button v-if="!suppressionOuverte" class="text-xs text-red-600 w-full text-center" @click="suppressionOuverte = true">
          Supprimer mon compte
        </button>

        <form v-else class="space-y-3" @submit.prevent="confirmerSuppression">
          <p class="text-xs text-red-600 font-medium">
            Cette action est définitive : votre profil, vos déclarations et vos messages seront
            supprimés. Saisissez votre mot de passe pour confirmer.
          </p>
          <input v-model="motDePasseSuppression" type="password" class="input-field" placeholder="Votre mot de passe" required autofocus />
          <p v-if="erreurSuppression" class="text-xs text-red-600">{{ erreurSuppression }}</p>
          <div class="flex gap-2">
            <button type="submit" class="btn-outline !border-red-300 !text-red-600 !py-2.5 text-sm flex-1" :disabled="suppressionEnCours">
              {{ suppressionEnCours ? 'Suppression…' : 'Confirmer la suppression' }}
            </button>
            <button type="button" class="btn-outline !py-2.5 text-sm" @click="suppressionOuverte = false">Annuler</button>
          </div>
        </form>
      </div>
    </div>
