...
const { confirmerVerificationTelephone, demanderVerificationTelephone } = useAuth()

const numero = ref(route.query.numero || '')
const code = ref('')
...
const verifier = async () => {
  ...
  loading.value = true
  try {
    await confirmerVerificationTelephone(numero.value, code.value)
    succes.value = true
...
const renvoyer = async () => {
  error.value = ''
  try {
    await demanderVerificationTelephone(numero.value)
    renvoye.value = true
...
<template>
  <div class="auth-split">
    <div class="auth-card">
    <AuthBrandPanel
      titre="Numéro vérifié"
      texte="Un numéro de téléphone vérifié renforce la confiance des autres membres dans vos déclarations."
    />
    ...
    <div class="auth-form-panel">
      <div class="w-full max-w-xs mx-auto">
        ...
        <h1 class="text-2xl font-bold mb-2">Vérifiez votre numéro</h1>
        <p class="text-forest-700/70 text-sm mb-8">
          Un code par SMS a été envoyé au <strong>{{ numero }}</strong>. Saisissez-le ci-dessous.
        </p>

        <form class="space-y-4" @submit.prevent="verifier">
          <div>
            <label class="label-field">Numéro de téléphone</label>
            <input v-model="numero" type="tel" class="input-field" required />
          </div>
          <div>
            <label class="label-field">Code reçu par SMS</label>
          ...
          <p v-if="succes" class="text-sm text-forest-600">Numéro vérifié ! Redirection…</p>
