...
// La carte "Activité récente" de l'accueil n'affiche que les premières lignes
// (le flux complet est consultable sur /activite-recente via "Voir tout") —
// on garde un aperçu concis sans surcharger la page d'accueil.
const NB_LIGNES_ACCUEIL = 7
const recentsApercu = computed(() => recents.value.slice(0, NB_LIGNES_ACCUEIL))
...
const chargerDonneesDirectes = async () => {
  ...
  try {
    const [stats, recentsRes] = await Promise.all([
      ...
      // Fil d'activité public : pertes ET trouvailles de toute la plateforme
      // (voir supabase/migration_25_activite_publique.sql), pas seulement
      // les objets trouvés comme avant.
      supabase.rpc('public_activity_feed', { p_limit: 8 })
    ])
    ...
    if (stats.data) {
      ...
      statsDirect.restitutions = stats.data.restitutions
    }
...
<template>
  <div>
    ...
    <div v-if="annonceActuelle" class="bg-forest-800 text-white py-2.5 px-4 overflow-hidden">
      ...
      >
        <NuxtLink :key="annonceActuelle.id" to="/recherche" class="flex items-center justify-center gap-2 text-xs sm:text-sm text-center hover:text-savane-300 transition-colors">
          ...
          <span class="truncate">
            {{ typeInfo(annonceActuelle.object_type_id).label }} {{ annonceActuelle.genre === 'perdu' ? 'perdu' : 'trouvé' }}
      ...
      <div class="container-app grid md:grid-cols-2 gap-10 xl:gap-20 items-center relative">
        ...
        <div class="relative flex justify-center">
          ...
          <template v-if="contenu.hero_background_type === 'video' && contenu.hero_video_url">
            <div
              class="relative z-10 w-full max-w-sm sm:max-w-md xl:max-w-lg aspect-[4/3] overflow-hidden rounded-3xl shadow-floating"
              :class="contenu.hero_backdrop_url ? 'ring-1 ring-white/15' : 'ring-1 ring-forest-800/5'"
            >
              <video
              ...
              <div class="absolute bottom-4 left-4 flex items-center gap-2 bg-white/95 backdrop-blur px-3 py-2 rounded-xl shadow-card">
    ...
    <!-- RECHERCHE RAPIDE -->
    <section v-reveal class="section -mt-8 md:-mt-10 relative z-10">
        ...
        <form class="flex flex-col sm:flex-row gap-3" @submit.prevent="lancerRechercheRapide">
          <div class="flex-1 flex items-center gap-3 bg-ivoire-100 px-4 py-3">
            ...
            <input
              ...
              class="bg-transparent w-full text-sm outline-none text-ink dark:text-ivoire-100 placeholder:text-forest-300 dark:placeholder:text-forest-500"
            />
          </div>
    ...
    <!-- BANNIÈRE D'ANNONCE -->
    <section v-reveal class="section pt-5 md:pt-6">
    ...
    <!-- TYPES D'OBJETS -->
    <section v-reveal class="section py-7 md:py-9">
      <div class="container-app">
    ...
    <!-- COMMENT CA MARCHE -->
    <section v-reveal class="section py-8 md:py-10 bg-forest-800 text-white">
      <div class="container-app">
        ...
        <div class="grid lg:grid-cols-5 gap-5">
          ...
          <div class="lg:col-span-3 card overflow-hidden">
            <div class="flex items-center justify-between bg-forest-800 text-white px-5 py-4">
              ...
              <NuxtLink to="/activite-recente" class="flex items-center gap-1 text-xs font-semibold text-forest-100/70 hover:text-white transition-colors">
          ...
          <div class="lg:col-span-2 card p-5 card-hover">
            ...
            <div class="mt-4 pt-4 border-t border-forest-50 flex items-center gap-3">
              <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-green-50 text-green-600">
                <IconTab name="handshake" class="h-4 w-4" />
              </span>
              <div class="min-w-0">
                <div v-if="chargementDonnees" class="h-6 w-10 bg-green-50 animate-pulse"></div>
                <div v-else class="text-xl font-display font-extrabold text-forest-800 tracking-tight leading-none">{{ statsDirect.restitutions ?? '—' }}</div>
                <div class="text-[11px] font-semibold uppercase tracking-wide text-forest-500 mt-1">Objets restitués avec succès</div>
              </div>
            </div>
    ...
    <!-- CARTE INTERACTIVE -->
    <section v-reveal class="section py-8 md:py-10 bg-forest-900 text-white mx-4 sm:mx-5 lg:mx-8">
    ...
    <section v-reveal class="section py-8 md:py-10">
      <div class="container-app grid lg:grid-cols-2 gap-8 lg:gap-12 items-start">
        ...
        <div class="grid sm:grid-cols-2 gap-4">
          <div v-for="(s, i) in securiteInfos" :key="s.titre" v-reveal="{ delay: i * 90 }" class="card p-5">
    ...
    <section v-reveal class="section py-8 md:py-10 bg-ivoire-100 mx-4 sm:mx-5 lg:mx-8">
      <div class="container-app grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
        <div class="order-2 lg:order-1 grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div v-for="(p, i) in partenairesTypes" :key="p.nom" v-reveal="{ delay: i * 70 }" class="card p-4 flex flex-col items-center text-center gap-2">
    ...
    <section v-reveal class="section py-7 md:py-9">
      <div class="container-app">
        ...
        <div class="grid sm:grid-cols-3 gap-5">
          <div v-for="(t, i) in temoignages" :key="t.nom" v-reveal="{ delay: i * 100 }" class="card p-6">
    ...
    <!-- CTA FINAL -->
    <section v-reveal class="section pb-8 md:pb-10">
      <div class="container-app bg-brand-gradient p-8 md:p-12 text-center text-white relative overflow-hidden">
        ...
        <div class="flex flex-wrap gap-3 justify-center relative">
          <NuxtLink to="/perdu" class="btn bg-white text-forest-800 hover:bg-ivoire-100"><IconTab name="search" class="h-4 w-4" /> J'ai perdu un objet</NuxtLink>
