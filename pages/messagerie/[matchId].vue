...
// Bug corrigé : quand les DEUX parties ont confirmé, aucun des deux ne doit
// voir "en attente de l'autre partie" — il faut un état de réussite dédié.
const restitutionTerminee = computed(() =>
  !!(restitution.value?.confirmee_par_proprietaire && restitution.value?.confirmee_par_trouveur)
)
const peutConfirmer = computed(() => {
  if (restitutionTerminee.value) return false
  return (estProprietaire.value && !restitution.value?.confirmee_par_proprietaire) ||
    (estTrouveur.value && !restitution.value?.confirmee_par_trouveur)
})
const dateRestitutionTexte = computed(() => {
  if (!restitution.value?.date_restitution) return ''
  return new Date(restitution.value.date_restitution).toLocaleDateString('fr-FR', {
    day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit'
  })
})

// Actualisation légère en arrière-plan : sans elle, une personne qui reste
// sur la page ne voit la confirmation de l'autre partie qu'après avoir
// rechargé manuellement. On rafraîchit messages/vérification/restitution
// toutes les 8 secondes tant que la conversation est ouverte.
let intervalActualisation = null
const actualiserEnArrierePlan = async () => {
  if (!supabase || !match.value) return
  const { data: msgs } = await supabase
    .from('messages')
    .select('id, expediteur_id, destinataire_id, contenu, lu, created_at')
    .eq('match_id', route.params.matchId)
    .order('created_at', { ascending: true })
  if (msgs && msgs.length !== messages.value.length) {
    messages.value = msgs
    scroll()
  }
  await Promise.all([chargerVerification(), chargerRestitution()])
...
onMounted(charger)
onMounted(() => {
  intervalActualisation = setInterval(actualiserEnArrierePlan, 8000)
})
onBeforeUnmount(() => { if (intervalActualisation) clearInterval(intervalActualisation) })
</script>
          ...
          <div class="md:col-span-2 space-y-4">
            ...
            <template v-else-if="verifRequest.validee === true">
              ...
              <div v-else class="card p-5">
                ...
                <div v-if="restitutionTerminee" class="rounded-xl bg-forest-50 text-forest-700 text-sm text-center py-3 px-4">
                  <p class="font-semibold flex items-center justify-center gap-1.5">
                    <IconTab name="check" class="h-4 w-4" /> Restitution confirmée par les deux parties 🎉
                  </p>
                  <p v-if="dateRestitutionTexte" class="text-xs text-forest-500 mt-1">{{ dateRestitutionTexte }}</p>
                </div>
                <button
                  v-else-if="peutConfirmer"
