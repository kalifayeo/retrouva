...
## 8quater. Dons : reçus automatiques, historique, export, push, attestations

Exécutez `supabase/migration_28_recus_historique_push_attestations.sql` (additive, sans risque sur
une base déjà à jour jusqu'à la migration 27) pour activer ce qui suit.

- **Reçu automatique (e-mail + SMS)** — envoyé dès qu'un don passe au statut « confirmé », que ce
  soit manuellement depuis `/admin/dons` ou automatiquement via un paiement en ligne
  Orange/MTN/Wave. Configuration optionnelle dans `.env` :
  - `RESEND_API_KEY` + `RESEND_FROM_EMAIL` (compte [resend.com](https://resend.com), domaine
    d'expédition vérifié) pour l'e-mail ;
  - `SMS_API_URL` (+ `SMS_API_KEY`, `SMS_SENDER`) pour le SMS — passerelle générique acceptant
    `{ to, message }`, à adapter dans `server/utils/envoyerSms.js` selon votre fournisseur.
  Tant que ces variables sont vides, le don se confirme normalement, seul l'envoi du reçu est
  sauté (un donateur connecté reçoit malgré tout une notification in-app). Depuis `/admin/dons`,
  vous voyez si le reçu a été envoyé et pouvez le renvoyer manuellement.
- **Historique des dons pour un donateur connecté** — un don fait en étant connecté est relié au
  compte et consultable sur `/mes-dons` (lien dans le menu du compte). Un don fait sans être
  connecté reste anonyme, comme avant.
- **Export CSV** — bouton « Export CSV » dans `/admin/dons`, ouvrable directement dans Excel/Google
  Sheets/LibreOffice (aucune dépendance supplémentaire).
- **Notifications push réelles (app mobile)** — relie enfin la dépendance
  `@capacitor/push-notifications` (présente mais inutilisée jusqu'ici) à un vrai envoi :
  `composables/usePushNotifications.js` enregistre l'appareil et sauvegarde son jeton (table
  `push_tokens`) dès qu'un compte est connecté sur l'app native ; `server/utils/envoyerPush.js`
  envoie via Firebase Cloud Messaging (API HTTP v1). Configuration : générez une clé de compte de
  service dans Console Firebase → Paramètres du projet → Comptes de service, et collez le JSON
  obtenu (une seule ligne) dans `FIREBASE_SERVICE_ACCOUNT_JSON`. Vide par défaut : les
  notifications in-app (cloche du header) continuent de fonctionner sans ce réglage.
- **Attestation / reçu de don (PDF)** — génération à la volée (sans stockage), téléchargeable
  depuis `/mes-dons` (donateur, pour ses propres dons confirmés) ou `/admin/dons` (équipe, pour
  n'importe quel don confirmé, y compris ceux des entreprises partenaires). Chaque don garde un
  numéro d'attestation stable (`RETROUVA-AAAA-NNNNNN`). Ce document reste un simple reçu de
  versement, pas une attestation fiscale au sens légal (RETROUVA n'ayant pas d'agrément en ce
  sens) — adaptez `server/utils/attestationPdf.js` si cette situation change.
- **CGU et politique de confidentialité** mises à jour avec une section dédiée aux dons/paiements
  (données bancaires traitées par l'opérateur choisi, durée de conservation, politique de
  remboursement, reçu/attestation).
